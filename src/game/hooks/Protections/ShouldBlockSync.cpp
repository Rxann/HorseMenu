...
namespace
{
	...
	std::unordered_set<uint32_t> g_RateLimitedModels = {0x8FF1C8C7, 0x1B51C381, 0x87B5AF59, 0x1B51C381, 0xB86499B8, 0x22E8E5BD, 0xA5B9ADD9};

	RateLimiter g_model_spam_limiter{30s, 1};
	RateLimiter g_guama_ship_limiter{30s, 1};
	...
	bool ShouldBlockNode(CProjectBaseSyncDataNode* node, SyncNodeId id, eNetObjType type, rage::netObject* object)
	{
		...
		case "CPedCreationDataNode"_J:
		{
			...
			if (data.m_ModelHash && g_RateLimitedModels.count(data.m_ModelHash))
			{
				g_model_spam_limiter.process();

				if (g_model_spam_limiter.exceeded_last_process())
				{
					LOG(WARNING) << "Blocked Potential Ped Spam Crash from" << Protections::GetSyncingPlayer().GetName();
					Notifications::Show("Protections",
					    std::string("Blocked Potential Ped Spam Crash from").append(Protections::GetSyncingPlayer().GetName()),
					    NotificationType::Warning);
					return true;
				}
			}
		...
		case "CVehicleCreationDataNode"_J:
		{
			...
			{
				g_guama_ship_limiter.process();

				if (g_guama_ship_limiter.exceeded_last_process())
				{
		...
		case "CPhysicalAttachDataNode"_J:
		{
			...
			if (auto local = Pointers.GetLocalPed(); local && local->m_NetObject)
			{
				...
				{
					LOG(WARNING) << "Blocked invalid physical attachment crash from " << Protections::GetSyncingPlayer().GetName();
					Notifications::Show("Protections",
					    std::string("Blocked invalid physical attachment crash from ")
		...
		case "CPedAttachDataNode"_J:
		{
			...
			if (auto local = Pointers.GetLocalPed(); local && local->m_NetObject)
			{
				...
				if (data.m_IsAttached && object && object->m_ObjectType == (uint16_t)eNetObjType::Trailer)
				{
					LOG(WARNING) << "Blocked invalid ped attachment crash from " << Protections::GetSyncingPlayer().GetName();
					Notifications::Show("Protections",
					    std::string("Blocked invalid ped attachment crash from ").append(Protections::GetSyncingPlayer().GetName()),
					    NotificationType::Warning);
					return true;
				}
			}
